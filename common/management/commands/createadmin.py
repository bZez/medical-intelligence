    ...
    def handle(self, *args, **options):
        ...
        if not interactive:
            try:
                ...
                print self.UserModel.REQUIRED_FIELDS
            ...
            try:
                ...
                while username is None:
                    if not username:
                        ...
                        raw_value = raw_input(force_str('%s: ' % input_msg))
                ...
                while password is None:
                    ...
                    if password.strip() == '':
                        ...
                        continue
                while otp is None:
                    otp = getpass.getpass(force_str('Press key on the key: '))
                    if '' == otp.strip():
                        self.stderr.write("Error: Blank key isn't allowed.")
                        otp = None
                        continue
                    if 12 > len(otp):
                        self.stderr.write("Error: UID is too short")
                        otp = None
                        continue
